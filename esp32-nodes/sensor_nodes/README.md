...
As of the current mesh protocol (`v3.2.1`), nodes are **self-describing**. Each node sends a sensor schema to the gateway at pair time that defines the labels, units, and precision of its readings. The gateway and web dashboard have no hardcoded knowledge of any specific sensor type and adapt automatically. Adding a new sensor to a node still requires only node firmware changes when the schema contract is preserved.

Current sensor-node firmwares also report a hardware configuration ID during registration so the gateway can block incompatible Node OTA images before delivery starts.
...
|-----------|---------|--------------|-------|
| `envo_mini_v1/` | Bosch BMP280 + DHT22 + TEMT6000 | Temperature, Atmospheric Pressure, Humidity, Ambient Light | DFRobot Firebeetle 2 ESP32-E (`v2.1.1`) |
